...
def _load_dior_obb_xml(xmlfile, cls2lbl):
    ...
    if xmlfile is None:
        ...
        for obj in root.findall('object'):
            ...
            bboxes.append([
                # float(bnd_box.find('x_left_top').text)-1,
                # float(bnd_box.find('y_left_top').text)-1,
                # float(bnd_box.find('x_right_top').text)+1,
                # float(bnd_box.find('y_right_top').text)-1,
                # float(bnd_box.find('x_right_bottom').text)-1,
                # float(bnd_box.find('y_right_bottom').text)+1,
                # float(bnd_box.find('x_left_bottom').text)+1,
                # float(bnd_box.find('y_left_bottom').text)+1,
